...
public class CustomSecurityConfig {
    ...
    public SecurityFilterChain FilterChain(HttpSecurity http) throws Exception {
        ...
        http.rememberMe(httpSecurityRememberMeConfigurer -> {
            ...
            log.info("--자동 로그인 기법 rememberMe--");
            //                            초 분 시 일 쿠기의 maxAge()
        });
        // p718 403예외처리 핸들러
        http.exceptionHandling(httpSecurityExceptionHandlingConfigurer -> {
            httpSecurityExceptionHandlingConfigurer.accessDeniedHandler(accessDeniedHandler()); // 하단에 메서드 추가
        });
    ...
    @Bean // 내장된 403가 아니라 내가 만든 재정의용 403 핸들러
    ...
    public PersistentTokenRepository persistentTokenRepository() {
        ...
        log.info("--토큰 생성 기법--");
        https:
//docs.spring.io/spring-security/site/docs/current/api/org/springframework/security/web/authentication/rememberme/PersistentTokenRepository.html
